...
set "CC_LOG_SOURCE=root/run.bat"
call :CC_RESOLVE_LOG_FILE "%CC_ENV_PATH%" "ROOT_RUN_BAT_LOG_FILE" "logs\casino_calendar_batch_run.log" "%CC_ROOT_DIR%"

call :CC_LOG INFO Starting Casino Calendar Application...
call "scripts\windows\run_direct.bat"
set "CC_EXIT_CODE=%ERRORLEVEL%"
if "%CC_EXIT_CODE%"=="0" (
    call :CC_LOG INFO Application run completed.
) else (
    call :CC_LOG ERROR Application run failed with exit code %CC_EXIT_CODE%.
)
exit /b %CC_EXIT_CODE%
