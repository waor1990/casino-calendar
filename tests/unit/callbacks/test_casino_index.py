...
class DummyCtx:
    def __init__(self, triggered_id):
        ...
        self.triggered = [{"prop_id": f"{triggered_id}.n_clicks", "value": 1}] if triggered_id else []
...
def test_casino_index_modal_toggle(monkeypatch, casino):
    ...
    monkeypatch.setattr("dash.callback_context", DummyCtx("open-casino-index-modal"), raising=False)
    class_name, style = callback(1, 0)
    assert class_name == "modal show"
    assert style == {}

    monkeypatch.setattr("dash.callback_context", DummyCtx("close-casino-index-modal"), raising=False)
