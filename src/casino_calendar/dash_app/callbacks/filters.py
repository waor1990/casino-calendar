...
def register_callbacks(app, df) -> None:
    ...
    def update_week_label(week_offset: int) -> html.Div | str:
        ...
        try:
            ...
            label_range = f"{week_start_pdt.strftime('%B %d')} - {week_end_pdt.strftime('%B %d')}"
    ...
    def toggle_casino_filter(n_clicks, ids, selected):
        ...
        available_indices = [item.get("index") for item in ids if isinstance(item, dict) and item.get("index")]
    ...
    def update_event_type_options(
        ...
        try:
            ...
            for offer_type in all_offer_types:
                ...
                options.append({"label": f"{emoji} {offer_type} ({count})", "value": offer_type})
