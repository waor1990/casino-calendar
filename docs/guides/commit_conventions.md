...
Run `npm run commit` (or `npx git-cz`) to open the Commitizen prompt powered by `.cz-config.js`. It presents the allowed types and scopes above and formats the commit message to satisfy commitlint and the Husky `commit-msg` hook.
