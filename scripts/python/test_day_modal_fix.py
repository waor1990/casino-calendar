...
try:
    ...
    if len(result) >= 2:
        ...
        print(
            f"✅ Header text: {header.children if hasattr(header, 'children') else 'OK'}"
        )
        print(
            f"✅ Grid container with class: {grid.className if hasattr(grid, 'className') else 'day-grid'}"
        )
        ...
        if hasattr(grid, "style") and "height" in grid.style:
            ...
            if height_px <= expected_max:
                print(
                    f"✅ Grid height {height_px}px is within expected range (≤{expected_max}px)"
                )
            else:
